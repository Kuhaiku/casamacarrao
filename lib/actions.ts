...
// Função auxiliar para mapear 1/0 para true/false
const mapBooleans = (obj: any) => {
  const newObj = { ...obj }
  for (const key in newObj) {
    if (typeof newObj[key] === 'number' && (newObj[key] === 1 || newObj[key] === 0)) {
      // Conversão condicional baseada em colunas que sabemos ser booleanas
...
export async function getStoreData() {
  ...
  return {
    ...
    orders: (orders as any[]).map(mapBooleans).map(o => ({ 
      ...
      items: typeof o.items === 'string' ? JSON.parse(o.items) : o.items // Trata JSON nativo do mysql2
    })),
    ...
    cashRegisters: (registers as any[]).map(r => ({ 
      ...r, 
      totalSales: Number(r.totalSales), totalExpenses: Number(r.totalExpenses), totalTips: Number(r.totalTips), netTotal: Number(r.netTotal),
...
export async function dbDispatch(action: string, payload: any) {
  switch (action) {
    case 'ADD_SIZE':
      await pool.query('INSERT INTO sizes (id, name, price, maxPastas, strictMaxPastas, maxIngredients, strictMaxIngredients, maxSauces, strictMaxSauces) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
    ...
    case 'ADD_MENU_ITEM':
      await pool.query('INSERT INTO menu_items (id, name, category, isActive) VALUES (?, ?, ?, ?)', 
      [payload.id || randomUUID(), payload.name, payload.category, payload.isActive])
      break
    ...
    case 'ADD_ORDER':
      await pool.query('INSERT INTO orders (id, customerName, phone, address, paymentMethod, status, isPaid, total, items) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', 
      [randomUUID(), payload.customerName, payload.phone, payload.address, payload.paymentMethod, payload.status, payload.isPaid, payload.total, JSON.stringify(payload.items)])
      break
    ...
    case 'CLOSE_REGISTER':
      ...
      try {
        ...
        await connection.query('INSERT INTO cash_registers (id, openedAt, totalSales, totalExpenses, totalTips, netTotal, orderCount) VALUES (?, ?, ?, ?, ?, ?, ?)', 
        [randomUUID(), settings[0].registerOpenedAt, totalSales, totalExpenses, totalTips, (totalSales + totalTips) - totalExpenses, activeOrders.length])
