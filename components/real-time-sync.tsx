...
import type { Size, MenuItem, StoreSettings, Order, StoreState, OrderStatus, FinancialEntry } from './types'
...
export const useStore = create<StoreState & StoreActions>((set, get) => ({
  sizes: [], menuItems: [], settings: { extraIngredientPrice: 3.00, extraCheesePrice: 8.00, whatsappMessage: "" },
  orders: [], expenses: [], tips: [], cashRegisters: [], registerOpenedAt: new Date().toISOString(),
  ...
  calculateOrderTotal: (items) => {
    ...
    for (const item of items) {
      ...
      if (!size) continue
      total += size.price
      if (!size.strictMaxIngredients) total += Math.max(0, item.ingredients.length - size.maxIngredients) * settings.extraIngredientPrice
      if (!size.strictMaxSauces) total += Math.max(0, item.sauces.length - size.maxSauces) * settings.extraIngredientPrice
      if (item.extraCheese) total += settings.extraCheesePrice
    }
    return total
  },
}))