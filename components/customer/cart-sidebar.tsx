      ...
      <div className="p-4 border-b border-stone-200 bg-white flex items-center justify-between shrink-0 z-10 shadow-sm">
        {step === 1 ? (
          <h2 className="text-lg font-black text-stone-800 flex items-center gap-2">
            <ShoppingBag className="w-5 h-5 text-orange-600" /> {stepTitles[step]}
          </h2>
        ) : (
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black text-stone-800">{stepTitles[step]}</h2>
          </div>
        )}
        <button
      ...
      <div className="flex-1 overflow-y-auto">
        {settings.isOpen === false && (
          <div className="p-4 bg-red-50 border-b border-red-100 text-red-800 flex items-start gap-2">
            ...
            <p className="text-sm font-bold">Nosso delivery está fora de atendimento no momento.</p>
          </div>
        )}
          ...
          <div className="p-4 space-y-6">
            ...
            {step === 1 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                {cartSelfService.map((item: any) => (
                  <div key={item.id} className="bg-white p-3.5 rounded-xl border border-stone-200 shadow-sm space-y-3">
                    <div className="flex justify-between items-start border-b border-stone-100 pb-2">
                      <span className="font-black text-stone-800 text-sm">
                        Macarrão {sizes.find((s: any) => s.id === item.sizeId)?.name}
                      </span>
                      <button onClick={() => handleRemoveSelfService(item.id)} className="text-stone-400 hover:text-red-500 bg-red-50/50 p-1.5 rounded-lg transition-colors">
                ...
                {cartAvulsos.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {cartAvulsos.map((item: any) => (
                      <div key={item.id} className="flex justify-between items-center bg-white p-3 rounded-xl border border-stone-200 shadow-sm">
                        <span className="text-sm font-bold text-stone-700">
                          {item.quantity}x {item.product.name}
                        </span>
                        <button onClick={() => handleRemoveAvulso(item.id)} className="text-stone-400 hover:text-red-500 p-1.5 rounded-lg bg-red-50/50">
                          <Trash2 className="w-4 h-4 text-red-500" />
                        </button>
                      </div>
                    ))}
            ...
            {step === 2 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-3">
                  <h3 className="font-black text-stone-800 text-sm flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-orange-600" /> Endereço de Entrega
                  </h3>
                  <div className="space-y-3 bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                    ...
                    >
                      ...
                      {isFetchingLocation ? "Buscando localização..." : "USAR MEU GPS"}
                    </button>
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      />
                      {isValidatingArea && <Loader2 className="w-4 h-4 animate-spin text-orange-600 shrink-0" />}
                    </div>
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-3">
                  <h3 className="font-black text-stone-800 text-sm flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-orange-600" /> Algo a declarar? <span className="font-normal text-stone-400 text-xs">(Opcional)</span>
                  </h3>
                  <textarea
            ...
            {step === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-3">
                  <h3 className="font-black text-stone-800 text-sm flex items-center gap-2">
                    <Banknote className="w-4 h-4 text-orange-600" /> Forma de Pagamento
                  </h3>
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    {settings.mercadoPagoAtivo && (
                      ...
                      >
                        ...
                        <span className="text-[11px] font-black text-center leading-tight">CARTÃO</span>
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm space-y-2 text-[13px] font-bold text-stone-500 uppercase">
                  <div className="flex justify-between">
                    <span>Subtotal:</span>
                    <span className="text-stone-800">{formatCurrency(cartSubtotal)}</span>
                  </div>
                  {taxaEmbalagem > 0 && (
                    <div className="flex justify-between">
                      <span>Embalagem:</span>
                      <span className="text-stone-800">{formatCurrency(taxaEmbalagem)}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span>Entrega:</span>
                    <span className="text-stone-800">{isValidatingArea ? "Calculando..." : formatCurrency(taxaEntrega)}</span>
                  </div>
                  {taxaCartao > 0 && (
                  ...
                  <div className="flex justify-between items-center pt-3 mt-3 border-t border-stone-100">
                    <span className="text-sm font-black text-stone-800">Total a Pagar</span>
                    <span className="text-2xl font-black text-orange-600">{formatCurrency(totalFinal)}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          {step === 1 && (
            <>
              <div className="flex justify-between items-center mb-3 px-1">
                ...
                <span className="text-2xl font-black text-stone-800">{formatCurrency(cartSubtotal)}</span>
              </div>
              <button
                onClick={() => setStep(2)}
                disabled={!settings.isOpen}
