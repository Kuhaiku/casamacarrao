    ...
    <Card className="shadow-lg rounded-2xl overflow-hidden flex flex-col bg-white dark:bg-stone-900 border-none h-[750px]">
      ...
      <CardContent className="p-4 sm:p-5 flex-1 flex flex-col overflow-hidden">
                ...
                >
                  ...
                  {item.extras?.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-amber-200 dark:border-amber-900/50">
                      <div className="text-xs font-bold text-amber-600 dark:text-amber-500 flex items-center gap-1.5 mb-0.5">
                        <Star className="w-3.5 h-3.5" /> Extras Pagos:
                      </div>
                      <div className="text-xs text-amber-800 dark:text-amber-400 pl-1 border-l-2 border-amber-300 dark:border-amber-700 ml-1.5 font-bold whitespace-normal break-words">
        ...
        <div className="shrink-0 space-y-3 mt-4 pt-4 border-t border-stone-100 dark:border-stone-800">
          ...
          <div className="flex justify-between items-center text-[10px] sm:text-[11px] text-stone-400 font-medium pt-1 pb-1">
          ...
          {/* PAINEL DE CONTROLE DE STATUS E PAGAMENTO ADICIONADO */}
          <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-stone-100 dark:border-stone-800">
            ...
            {order.status !== "entregue" && order.status !== "cancelado" && (
              <Button variant="ghost" size="sm" onClick={() => updateOrderStatus(order.id, "cancelado")} className="text-red-500 hover:text-red-700 hover:bg-red-50 ml-auto p-2" title="Cancelar Pedido">
                <Ban className="w-4 h-4" />
              </Button>
            )}

            {order.status === "cancelado" && (
              <Button variant="outline" size="sm" onClick={() => updateOrderStatus(order.id, "novo")} className="border-red-200 text-red-600 hover:bg-red-50 ml-auto font-bold bg-white">
                <RotateCcw className="w-4 h-4 mr-1" /> Reabrir Pedido
              </Button>
            )}
          </div>
        </div>
