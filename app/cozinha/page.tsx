    ...
    <Card className="border-2 border-primary/20 bg-card shadow-md flex flex-col h-full">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2 text-stone-600 dark:text-stone-400 font-medium">
              ...
              <Badge variant="secondary" className="font-bold">{getTimeSince(order.createdAt)}</Badge>
            </div>
          </div>
          <Badge variant="outline" className="text-lg px-3 py-1 font-black bg-white dark:bg-stone-900 border-stone-300">
            #{order.id.slice(0, 4)}
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="pt-4 space-y-4 flex-1 flex flex-col">
        ...
        {order.items && order.items.map((item: any, idx: number) => (
          <div key={`mac-${idx}`} className="p-4 bg-stone-100 dark:bg-stone-800/50 rounded-xl border border-stone-200 dark:border-stone-700">
            ...
            <div className="space-y-2 text-lg text-stone-700 dark:text-stone-300">
              {item.pastaId && (
                <div className="flex gap-2">
                  ...
                  <span className="font-medium">{getItemName(item.pastaId)}</span>
                </div>
              )}
              ...
              {item.sauces?.length > 0 && (
                <div className="flex gap-2">
                  ...
                  <span className="font-medium">{item.sauces.map(getItemName).join(", ")}</span>
                </div>
              )}
              ...
              {item.temperos?.length > 0 && (
                <div className="flex gap-2">
                  ...
                  <span className="font-medium">{item.temperos.map(getItemName).join(", ")}</span>
                </div>
              )}
              ...
              {item.ingredients?.length > 0 && (
                <div className="flex gap-2">
                  ...
                  <span className="font-medium">{item.ingredients.map(getItemName).join(", ")}</span>
                </div>
              )}
        ...
        {order.products && order.products.length > 0 && (
          <div className="p-4 bg-blue-50 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-900">
            <div className="text-sm font-black mb-2 text-blue-800 dark:text-blue-400 uppercase tracking-wider">Outros Itens:</div>
            <div className="space-y-1.5">
              {order.products.map((prod: any, idx: number) => (
                <div key={`prod-${idx}`} className="text-xl font-black text-blue-950 dark:text-blue-300 flex items-center gap-2">
                  <span className="bg-blue-200 dark:bg-blue-800 px-2 py-0.5 rounded text-blue-900 dark:text-blue-100">{prod.quantity}x</span>
        ...
        {order.observation && (
          <div className="p-4 bg-amber-100 dark:bg-amber-950/50 rounded-xl border-2 border-amber-400 dark:border-amber-700 shadow-sm">
            <div className="text-sm font-black mb-2 text-amber-900 dark:text-amber-500 uppercase flex items-center gap-1.5 tracking-wider">
              <AlertCircle className="w-5 h-5 text-amber-600 dark:text-amber-500" /> Observação do Cliente:
            </div>
        ...
        <div className="flex-1"></div> {/* Empurra o botão para baixo */}
...
export default function KitchenPage() {
  ...
  // A cozinha só enxerga os pedidos que já foram "aprovados" pelo caixa/sistema automático
      ...
      <header className="border-b bg-white dark:bg-stone-900 sticky top-0 z-10 shadow-sm">
        <div className="container flex h-20 items-center justify-between px-4 max-w-[1400px] mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-black text-2xl text-stone-800 dark:text-stone-100 leading-tight">Cozinha</h1>
              <p className="text-sm font-bold text-stone-500">Monitor de Preparo</p>
            </div>
            <Badge variant="secondary" className="ml-4 text-lg px-4 py-1 bg-stone-200 text-stone-700 dark:bg-stone-800 dark:text-stone-300 font-black">
