          ...
          {isCartEmpty ? (
            ...
            <>
              {cartSelfService.map((item: any) => {
                let sub =
                  sizes.find((s: any) => s.id === item.sizeId)?.price || 0;
...
function OrderBuilder({ db, onFinish, formatCurrency }: any) {
  ...
  if (selectedSize) {
    if (!selectedSize.strictMaxSauces)
      currentTotal +=
        Math.max(0, order.sauces.length - selectedSize.maxSauces) *
        db.settings.extraIngredientPrice;
    if (!selectedSize.strictMaxIngredients)
      currentTotal +=
        Math.max(0, order.ingredients.length - selectedSize.maxIngredients) *
        db.settings.extraIngredientPrice;
    if (order.extraCheese) currentTotal += db.settings.extraCheesePrice;
  }

  const massas = db.menuItems.filter(
    (i: any) => i.isActive && i.category === "pasta",
  );
  const molhos = db.menuItems.filter(
    (i: any) => i.isActive && i.category === "sauce",
  );
  const temperos = db.menuItems.filter(
    (i: any) => i.isActive && i.category === "seasoning",
  );
  const ingredientes = db.menuItems.filter(
    (i: any) => i.isActive && i.category === "ingredient",
  );
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-4 sm:p-6 shadow-sm mb-6 flex-1">
        {step === 0 && (
          <div className="animate-in fade-in duration-300">
            ...
            <p className="text-xs sm:text-sm text-stone-500 mb-4">
              Cada tamanho tem seus próprios limites.
            </p>
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5 sm:gap-3 mt-1.5 text-[10px] sm:text-xs text-stone-500 font-medium">
                        <span className="bg-white/50 px-1.5 rounded">
                          🍝 {sz.maxPastas} massa
                        </span>
                        <span className="bg-white/50 px-1.5 rounded">
                          🥫 {sz.maxSauces} molho{sz.maxSauces > 1 ? "s" : ""}
                        </span>
                        <span className="bg-white/50 px-1.5 rounded">
                          🥓 {sz.maxIngredients} ingr.
                        </span>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
              {massas.map((m: any) => (
                ...
                >
                  ...
                  <span
                    className={`text-[11px] sm:text-sm font-bold leading-tight ${order.pastaId === m.id ? "text-orange-900" : "text-stone-700"}`}
                  >
                    {m.name}
                  </span>
                </button>
        ...
        {step === 2 && (
          <div className="animate-in fade-in duration-300">
            ...
            <p className="text-xs sm:text-sm text-stone-500 mb-3 sm:mb-4">
              Adicionais custam{" "}
              {formatCurrency(db.settings.extraIngredientPrice)}.
            </p>
            {order.sauces.length > (selectedSize?.maxSauces || 1) &&
              !selectedSize?.strictMaxSauces && (
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isActive)
                        setOrd({
                          sauces: order.sauces.filter((id) => id !== m.id),
                        });
                      else if (
                        !selectedSize?.strictMaxSauces ||
                        order.sauces.length < selectedSize.maxSauces
                  ...
                  >
                    ...
                    <span
                      className={`text-[11px] sm:text-sm font-bold leading-tight ${isActive ? "text-red-900" : "text-stone-700"}`}
                    >
                      {m.name}
                    </span>
                    {isActive && (
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isActive)
                        setOrd({
                          temperos: order.temperos.filter((id) => id !== t.id),
                        });
                  ...
                  >
                    ...
                    <span
                      className={`text-[10px] sm:text-xs font-bold leading-tight ${isActive ? "text-green-900" : "text-stone-700"}`}
                    >
                      {t.name}
                    </span>
                  </button>
        ...
        {step === 4 && (
          <div className="animate-in fade-in duration-300">
            ...
            {order.ingredients.length > (selectedSize?.maxIngredients || 4) &&
              !selectedSize?.strictMaxIngredients && (
                <div className="bg-amber-50 border border-amber-200 text-amber-800 p-2.5 sm:p-3 rounded-xl flex items-center gap-2 mb-4 text-[11px] sm:text-sm font-medium">
                  <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600 shrink-0" />{" "}
                  Ingredientes extras (+
                  {formatCurrency(
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (isActive)
                        setOrd({
                          ingredients: order.ingredients.filter(
                            (id) => id !== i.id,
                          ),
                        });
                      else if (
                  ...
                  >
                    <span
                      className={`text-[10px] sm:text-xs font-bold leading-tight ${isActive ? "text-amber-900" : "text-stone-700"}`}
                    >
                      {i.name}
                    </span>
                    {isActive && (
            ...
            <button
              ...
              className={`w-full flex items-center justify-between p-3 sm:p-4 rounded-xl border-2 transition-all ${order.extraCheese ? "border-yellow-400 bg-yellow-50" : "border-stone-200 hover:border-yellow-300 bg-white"}`}
            >
              ...
              <div className="flex items-center gap-1 sm:gap-2">
                <span className="font-bold text-amber-600 text-xs sm:text-sm">
                  +{formatCurrency(db.settings.extraCheesePrice)}
                </span>
                {order.extraCheese && (
                  <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-yellow-400 flex items-center justify-center">
                    <Check className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" />
                  </div>
                )}
              </div>
