...
export default function CustomerHome() {
  ...
  // Carrinho Local (Separado entre Macarrão Montado e Produtos Prontos)
  ...
  // Agrupa Produtos (Bebidas, Sobremesas, etc) nas Categorias Reais do BD
  ...
  const handleAddAvulso = (produto: any) => {
    setCartAvulsos(prev => {
      ...
      if (existing) {
        return prev.map(item => item.productId === produto.id ? { ...item, quantity: item.quantity + 1 } : item)
      }
  ...
  const handleAddSelfService = (macarrao: any) => {
    setCartSelfService(prev => [...prev, { ...macarrao, id: Date.now().toString() }])
    setView("menu")
  }
  ...
  if (orderSuccess) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50 p-4">
        <div className="max-w-sm w-full text-center py-12 bg-white rounded-2xl shadow-sm border border-stone-200">
          <div className="w-20 h-20 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-5">
            <CheckCircle2 className="w-10 h-10 text-green-600" />
          </div>
          <h2 className="text-2xl font-bold text-stone-800 mb-2">Pedido Enviado!</h2>
          <p className="text-stone-500 text-sm mb-6">Sua solicitação foi enviada para a cozinha com sucesso.</p>
          <button
            onClick={() => window.location.reload()}
            className="px-6 py-3 rounded-xl font-semibold text-white transition bg-orange-700 hover:bg-orange-800 w-full"
          >
            Fazer Novo Pedido
          </button>
        </div>
  ...
  // ═══════════════════════════════════════════════════════════════
  // SACOLA LATERAL (SEMPRE VISÍVEL NO PC / MODAL NO MOBILE)
  // ═══════════════════════════════════════════════════════════════
  const CartContent = () => {
      ...
      <div className="flex flex-col h-full bg-white">
        ...
        <div className="p-4 sm:p-5 bg-stone-50 border-t border-stone-200 space-y-4">
          ...
          <div className="space-y-2 sm:space-y-3">
            <input
              type="text" placeholder="Seu Nome (Ex: João Silva)"
            ...
            />
            <input
              type="text" placeholder="Endereço Completo de Entrega"
            ...
            <div className="grid grid-cols-3 gap-1 sm:gap-2">
              {Object.entries(PAY_META).map(([key, { label, icon: Icon }]) => (
                <button
                  ...
                  disabled={isCartEmpty}
                  className={`flex flex-col items-center justify-center gap-1 p-2 rounded-lg border transition-all disabled:opacity-50 ${
                    payment === key && !isCartEmpty ? "border-orange-600 bg-orange-50 text-orange-700" : "border-stone-200 text-stone-500"
                  }`}
                >
  ...
  return (
    <div className="flex min-h-screen bg-stone-50 font-sans">
      ...
      {/* ═══════════════════════════════════════════════════════════════ */}
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isMobileCartOpen ? 'hidden lg:flex' : 'flex w-full'}`}>
        
        <div className="bg-stone-900 text-white px-4 sm:px-6 py-5 sm:py-6 shadow-md z-10 sticky top-0">
      ...
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* LADO DIREITO: CARRINHO FIXO NA TELA */}
      {/* ═══════════════════════════════════════════════════════════════ */}
      <aside className="hidden lg:flex w-[380px] xl:w-[420px] h-screen sticky top-0 border-l border-stone-200 shadow-2xl z-20 flex-col bg-white shrink-0">
        <CartContent />
      </aside>

      {/* CARRINHO MOBILE (Barra Inferior Fixa) */}
      {isMobileCartOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden flex flex-col bg-stone-900/50 backdrop-blur-sm animate-in fade-in">
          ...
          <div className="h-[90vh] bg-white rounded-t-3xl overflow-hidden shadow-2xl animate-in slide-in-from-bottom-full">
            <CartContent />
          </div>
        </div>
      ) : (
...
// ═══════════════════════════════════════════════════════════════
// WIZARD INTERNO DO SELF-SERVICE (MOBILE-FIRST)
// ═══════════════════════════════════════════════════════════════
function OrderBuilder({ db, onFinish, formatCurrency }: any) {
  ...
  return (
    <div className="max-w-xl mx-auto w-full">
      {/* Barra de Preço Fixa */}
      {selectedSize && (
        <div className="sticky top-0 z-20 mb-4 bg-stone-900 rounded-xl px-4 py-3 flex items-center justify-between shadow-lg">
          <div>
      ...
      {/* Indicador de Passos (Scrollável no Mobile) */}
      <div className="flex items-center gap-1 sm:gap-2 mb-4 sm:mb-6 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] whitespace-nowrap w-full">
        {STEPS.map((name, i) => (
          <div key={i} className="flex items-center shrink-0">
            <div className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full text-[10px] sm:text-xs font-bold transition-all ${
        ...
        {step === 1 && (
          <div className="animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
              {massas.map((m:any) => (
                <button
                  key={m.id} onClick={() => setOrd({ pastaId: m.id })}
              ...
              {massas.length === 0 && <p className="col-span-2 sm:col-span-3 text-xs sm:text-sm text-stone-400 text-center py-4">Nenhuma massa disponível.</p>}
            </div>
          </div>
        )}
          ...
          <div className="animate-in fade-in duration-300">
            ...
            {order.sauces.length > (selectedSize?.maxSauces||1) && !selectedSize?.strictMaxSauces && (
              <div className="bg-amber-50 border border-amber-200 text-amber-800 p-2.5 sm:p-3 rounded-xl flex items-center gap-2 mb-4 text-[11px] sm:text-sm font-medium">
                ...
                Molhos extras (+{formatCurrency((order.sauces.length - selectedSize!.maxSauces) * db.settings.extraIngredientPrice)})
              </div>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      else if (!selectedSize?.strictMaxSauces || order.sauces.length < selectedSize.maxSauces) {
                        setOrd({ sauces: [...order.sauces, m.id] })
                      }
                    }}
              ...
              {molhos.length === 0 && <p className="col-span-2 sm:col-span-3 text-xs sm:text-sm text-stone-400 text-center py-4">Nenhum molho disponível.</p>}
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
              ...
              {temperos.length === 0 && <p className="col-span-2 sm:col-span-3 text-xs sm:text-sm text-stone-400 text-center py-4">Nenhum tempero disponível.</p>}
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="animate-in fade-in duration-300">
            ...
            {order.ingredients.length > (selectedSize?.maxIngredients||4) && !selectedSize?.strictMaxIngredients && (
              <div className="bg-amber-50 border border-amber-200 text-amber-800 p-2.5 sm:p-3 rounded-xl flex items-center gap-2 mb-4 text-[11px] sm:text-sm font-medium">
                <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600 shrink-0" />
                Ingredientes extras (+{formatCurrency((order.ingredients.length - selectedSize!.maxIngredients) * db.settings.extraIngredientPrice)})
              </div>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3 mb-5 sm:mb-6">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      else if (!selectedSize?.strictMaxIngredients || order.ingredients.length < selectedSize.maxIngredients) {
                        setOrd({ ingredients: [...order.ingredients, i.id] })
                      }
                    }}
              ...
              {ingredientes.length === 0 && <p className="col-span-2 sm:col-span-3 text-xs sm:text-sm text-stone-400 text-center py-4">Nenhum ingrediente disponível.</p>}
            </div>

            <button
              onClick={() => setOrd({ extraCheese: !order.extraCheese })}
              className={`w-full flex items-center justify-between p-3 sm:p-4 rounded-xl border-2 transition-all ${
                order.extraCheese ? "border-yellow-400 bg-yellow-50" : "border-stone-200 hover:border-yellow-300 bg-white"
              }`}
            >
      ...
      <div className="flex gap-2 sm:gap-3">
        {step > 0 && (
          <button onClick={() => setStep(s => s - 1)} className="px-4 sm:px-6 py-2.5 sm:py-3 border border-stone-300 rounded-xl font-bold text-stone-600 hover:bg-stone-100 text-xs sm:text-sm">
            Voltar
          </button>
        )}
